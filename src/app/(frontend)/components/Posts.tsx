...
const Posts = ({ posts }: { posts: Post[] }) => {
  return (
    <div
      dir="ltr"
      className="grid grid-cols-3 gap-[0.8px] bg-base-200 items-center rounded-box border border-accent w-full overflow-hidden"
    >
      {posts?.map((post, idx) => (
        <Link
          href={`/posts/${post?.id}`}
          key={idx}
          className="w-full max-w-sm bg-gray-200 aspect-9/16 relative group"
        >
